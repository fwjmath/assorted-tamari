...
def checkpath(p):
    ...
    if len(p) != p.count(0) + p.count(1):
        raise ValueError("The input must be a list of 0 (east steps) "
                         "and 1 (north step).")
    return
...
def NuTamariLattice(v):
    ...
    return LatticePoset(dict((e, get_cover_elem(e))
                             for e in generate_elements()))
...
def pathpair_to_dyck(u, v):
    ...
    Dyck path that is the corresponding element
    in the classical Tamari lattice.
    ...
    for i in range(h):
        if ulvl[i] > vlvl[i]:
            raise ValueError("The first path should be weakly above "
                             "the second path.")
